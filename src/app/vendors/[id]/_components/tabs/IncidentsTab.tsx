...
function CreateIncidentForm({
  ...
  return (
    <form action={formAction} className="bg-white rounded-2xl p-5 space-y-4 mb-6" style={{ boxShadow: '0 2px 12px rgba(109,93,211,0.08)', border: '1px solid rgba(109,93,211,0.1)' }}>
      ...
      {/* Row 3: Impact (single dropdown — details added later via Edit) */}
      <div>
        <label className={labelCls}>Impact Scope</label>
        <select name="impact_scope" defaultValue="none" className={inputCls} style={{ maxWidth: 240 }}>
      ...
      <p className="text-xs" style={{ color: '#6b5fa8' }}>
        Records affected, response timeline, root cause, and regulatory details can be added after raising — open the incident and click Edit.
      </p>
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button type="button" onClick={onClose} className="text-sm text-[#6b5fa8] hover:text-[#6c5dd3] px-2 py-2">
          Cancel
        </button>
      </div>
    </form>
    ...
    <form action={formAction} className="mt-3 p-4 rounded-lg space-y-3" style={{ background: 'rgba(109,93,211,0.03)', border: '1px solid rgba(109,93,211,0.1)' }}>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div>
          ...
          <select name="incident_type" defaultValue={incident.incident_type ?? 'other'} className={inputCls}>
            {Object.entries(INCIDENT_TYPE_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div>
          <label className={labelCls}>Impact Scope</label>
          <select name="impact_scope" defaultValue={incident.impact_scope ?? 'none'} className={inputCls}>
            <option value="none">None</option>
            <option value="limited">Limited</option>
            <option value="moderate">Moderate</option>
            <option value="significant">Significant</option>
        ...
        <div>
          ...
          <input name="records_affected" type="number" min="0" defaultValue={incident.records_affected ?? ''} placeholder="0" className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Users Affected</label>
          <input name="users_affected" type="number" min="0" defaultValue={incident.users_affected ?? ''} placeholder="0" className={inputCls} />
        </div>
        <div>
          <label className={labelCls}>Data Types Involved</label>
          <input name="data_types_involved" type="text" defaultValue={incident.data_types_involved ?? ''} placeholder="personal, financial…" className={inputCls} />
        </div>
      </div>

      {/* Timeline + Root Cause */}
      <details className="text-xs" open>
        <summary className="cursor-pointer text-xs font-bold uppercase tracking-widest" style={{ color: '#5d5285' }}>
          Response Timeline + Root Cause
        </summary>
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <label className={labelCls}>Detected At</label>
            <input name="detected_at" type="datetime-local" defaultValue={incident.detected_at ? incident.detected_at.slice(0, 16) : ''} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Vendor Reported At</label>
            <input name="reported_by_vendor_at" type="datetime-local" defaultValue={incident.reported_by_vendor_at ? incident.reported_by_vendor_at.slice(0, 16) : ''} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Root Cause</label>
            <select name="root_cause" defaultValue={incident.root_cause ?? ''} className={inputCls}>
          ...
          <div>
            ...
            <input name="sla_notification_hours" type="number" min="0" defaultValue={incident.sla_notification_hours ?? ''} placeholder="e.g. 72" className={inputCls} />
          </div>
        </div>
        <div className="mt-3">
          <label className={labelCls}>Root Cause Detail</label>
          <textarea name="root_cause_detail" rows={2} defaultValue={incident.root_cause_detail ?? ''} placeholder="What caused this incident?" className={inputCls} />
        </div>
      </details>
      ...
      <details className="text-xs">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
          <div className="flex items-center gap-2 pt-5">
            <input name="is_reportable" type="checkbox" value="true" defaultChecked={incident.is_reportable} className="h-4 w-4 rounded" style={{ accentColor: '#6c5dd3' }} />
          ...
          <div>
            ...
            <input name="applicable_regulation" type="text" defaultValue={incident.applicable_regulation ?? ''} placeholder="GDPR, HIPAA…" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Reporting Deadline</label>
            <input name="reporting_deadline" type="datetime-local" defaultValue={incident.reporting_deadline ? incident.reporting_deadline.slice(0, 16) : ''} className={inputCls} />
          </div>
        </div>
      </details>
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <button type="button" onClick={onClose} className="text-xs text-[#6b5fa8] hover:text-[#6c5dd3] px-2 py-2">
          Cancel
        </button>
      </div>
    </form>
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        disabled={isPending}
        className="inline-flex items-center gap-1 text-xs text-[#6b5fa8] hover:text-rose-600 font-medium shrink-0 transition-colors disabled:opacity-50"
      >
      ...
      <div className="px-5 py-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap">
            {incType && (
              <span className="text-xs px-2 py-0.5 rounded font-bold uppercase" style={{ background: 'rgba(109,93,211,0.06)', color: '#6c5dd3' }}>
                {incType}
              </span>
            )}
            ...
            {slaBreached === true && (
              <span className="text-xs px-1.5 py-0.5 rounded font-bold uppercase" style={{ background: 'rgba(225,29,72,0.1)', color: '#e11d48' }}>SLA Breached</span>
            )}
            {incident.is_reportable && !incident.reported_to_regulator && (
              <span className="text-xs px-1.5 py-0.5 rounded font-bold uppercase" style={{ background: 'rgba(245,158,11,0.1)', color: '#d97706' }}>Reporting Required</span>
            )}
            {recurrenceCount > 0 && (
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button onClick={() => setEditing((e) => !e)} className="text-xs text-[#6b5fa8] hover:text-[#6c5dd3] font-medium transition-colors">
              {editing ? 'Close' : 'Edit'}
            </button>
        ...
        {(incident.records_affected || incident.users_affected || incident.impact_scope) && (
          <div className="flex items-center gap-3 mt-2 text-xs" style={{ color: '#4a4270' }}>
        ...
        {(incident.detected_at || incident.reported_by_vendor_at || incident.contained_at || incident.resolved_at) && (
          <div className="flex items-center gap-4 mt-3 text-xs flex-wrap" style={{ color: '#5d5285' }}>
        ...
        <p className="mt-3 text-xs" style={{ color: '#6b5fa8' }}>
    ...
    <div className="space-y-6">
      ...
      {active.length > 0 && (
        <section className="space-y-3">
          <h4 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#e11d48' }}>Active ({active.length})</h4>
          {active.map((incident) => (
            <IncidentCard
              key={incident.id}
              incident={incident}
      ...
      {resolved.length > 0 && (
        <section className="space-y-3">
          <h4 className="text-xs font-bold uppercase tracking-widest" style={{ color: '#059669' }}>Resolved / Closed ({resolved.length})</h4>
        ...
        return (
          <div key={step.key} className="flex items-center flex-1">
            <div className="flex flex-col items-center">
              ...
              <span className="text-[8px] mt-0.5 font-medium" style={{ color: isDone ? '#059669' : isCurrent ? '#6c5dd3' : '#6b5fa8' }}>
                {step.label}
              </span>
            </div>
